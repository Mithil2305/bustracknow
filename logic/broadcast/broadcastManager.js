...
class BroadcastManager {
  ...
  async startBroadcast(userId, routeId, onLocationUpdate = () => {}) {
    ...
    try {
      ...
      const locationEnabled = await RNAndroidLocationEnabler.promptForEnableLocationIfNeeded({
        interval: 10000,
        fastInterval: 5000,
      });
      ...
      this.broadcastInterval = setInterval(async () => {
        try {
          ...
          if (this.tripMinutes > 0 && this.tripMinutes * 5 > this.pointsAwarded) {
      ...
      console.log(`✅ Broadcast started for route ${routeId} (session: ${this.sessionId})`);
      return true;
    } catch (error) {
